...
export default function ScreenQuedas2({ formData, setFormData, nextStep, prevStep }) {
  // DEBUG: Verificar se as props estão chegando
  useEffect(() => {
    console.log("🔍 ScreenQuedas2 - Props recebidas:", {
  ...
  const [checklistItems, setChecklistItems] = useState([
    ...
    { id: 2, label: "Trocado de tomada", checked: false, text: "Realizado troca de tomada do equipamento" },
    { id: 3, label: "Alteração de Canal", checked: false, text: "Alteração de canal realizada" },
    { id: 4, label: "Alteração de Largura de Banda", checked: false, text: "Alteração de largura de banda realizada" },
    { id: 5, label: "Reiniciado os Equipamentos", checked: false, text: "Equipamentos reiniciados" },
    { id: 6, label: "Habilitado/Desabilitado IPv6", checked: false, text: "IPv6 habilitado/desabilitado" }
  ]);

  // Estado para os novos campos de validação
  const [validacoes, setValidacoes] = useState({
    verificadoQuedasIXC: formData.verificadoQuedasIXC || "",
    quedasOutrosEquipamentos: formData.quedasOutrosEquipamentos || "",
    quedasProximoEquipamento: formData.quedasProximoEquipamento || "",
  ...
  const buildValidacoesText = (validacoes) => {
    ...
    // Verificado quedas no IXC e OLT
    if (validacoes.verificadoQuedasIXC) {
      parts.push(`- Foi verificado as quedas no diagnostico do IXC e na OLT? ${validacoes.verificadoQuedasIXC}`);
    }
    
    // Quedas em outros equipamentos
    if (validacoes.quedasOutrosEquipamentos) {
      parts.push(`- Cliente tem quedas em outros equipamentos? ${validacoes.quedasOutrosEquipamentos}`);
    }
    
    // Quedas próximo do equipamento
    if (validacoes.quedasProximoEquipamento) {
      parts.push(`- Cliente tem quedas mesmo proximo do equipamento? ${validacoes.quedasProximoEquipamento}`);
    }
      ...
      <div className="space-y-6 flex-1">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Foi verificado as quedas no diagnostico do IXC e na OLT?
              </label>
              <select
                value={validacoes.verificadoQuedasIXC}
                onChange={(e) => handleValidacaoChange('verificadoQuedasIXC', e.target.value)}
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Cliente tem quedas em outros equipamentos?
              </label>
              <select
                value={validacoes.quedasOutrosEquipamentos}
                onChange={(e) => handleValidacaoChange('quedasOutrosEquipamentos', e.target.value)}
                className="w-full bg-white text-gray-800 px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-200 focus:border-blue-500 transition-all duration-200"
              >
                <option value="">Selecione</option>
                <option value="Sim">Sim</option>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Cliente tem quedas mesmo proximo do equipamento?
              </label>
              <select
                value={validacoes.quedasProximoEquipamento}
                onChange={(e) => handleValidacaoChange('quedasProximoEquipamento', e.target.value)}
                className="w-full bg-white text-gray-800 px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-200 focus:border-blue-500 transition-all duration-200"
              >
                <option value="">Selecione</option>
                <option value="Sim">Sim</option>
        ...
        <div className="bg-gray-50 rounded-xl p-5 border border-gray-200">
          ...
          <p className="text-sm text-gray-600 italic">
            ...
            - Cliente tem quedas mesmo proximo do equipamento? Sim"
          </p>
        </div>
